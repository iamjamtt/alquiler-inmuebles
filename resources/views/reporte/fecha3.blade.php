        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="p-6 bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-gray-200 flex justify-center">
                    <div class="md:w-3/4">
                        @csrf
                        <div class="text-xl flex justify-center font-bold">Reporte de Locales dados de Alta</div>
                        <!-- Validation Errors -->
                        <x-auth-validation-errors class="mb-4" :errors="$errors" />
                        <div class="space-y-3 mt-6">
                            <form method="POST" action="{{ route('reporte.fecha3') }}">
                            @csrf
                                    ...
                                    <tbody>
                                        @foreach ($local as $item)
                                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                                ...
                                                <th scope="row" class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                                    Piso N°{{$item->pisos->nombre}}
                                                </th>
                                                <th scope="row" class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                                    $ {{$item->precio}}
                                                </th>
                                                <th scope="row" class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                                    {{$item->pisos->edificio->nombre}}
                                                </th>
                                                <th scope="row" class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                                    {{$item->pisos->edificio->TipoEdificio->tipo_edificio}}
                                                </th>
                                            </tr>
                                        @endforeach
                                    </tbody>
